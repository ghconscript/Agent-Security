...
def run_chatbot():
    ...
    baserun.register_template(
        template_name="Customer Service", template_messages=[{"role": "system", "content": PROMPT}]
    )
    ...
    with baserun.start_trace(name="Chatbot CLI loop") as trace:
        while user_input != EXIT:
            formatted_conversation = baserun.format_prompt("Customer Service", {}, conversation)
        ...
        return annotation.submit()

        # Completion not showing up in template
