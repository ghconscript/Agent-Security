...
async def run_schema_spoof_experiment():
    ...
    print("-" * 80)
    with open(poisoned_readme, 'r', encoding='utf-8') as f:
        content = f.read()
        # Show first forged trace (ReAct "Thought:" or MetaGPT-style "Code executed:" / 危险命令)
        if "Thought:" in content:
            start = content.find("Thought:")
            snippet = content[start:start+400]
            print(snippet[:400] + "...\n")
            print(f"✓ Forged ReAct traces detected in README")
        elif "Code executed:" in content or "bootstrap.internal" in content or "deploy --skip-tests" in content:
            print("(README 内含伪造的部署/代码执行痕迹，格式为 task_id + Code executed)\n")
            print("✓ Forged deployment/code traces detected in README")
        else:
            print("⚠️  No ReAct-style or deployment-spoof traces found in README")
