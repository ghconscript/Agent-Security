...
def build_engine(experiences: Iterable[ExperienceDoc]) -> SimpleEngine:
    ...
    Uses BM25-only to avoid embedding API (DeepSeek etc. may not expose embeddings or use different model names).
    """
    retriever_configs = [BM25RetrieverConfig(create_index=True, similarity_top_k=3)]
    print("[OK] Using BM25-only retriever (no embedding API required).")
...
async def run():
    ...
    clean_persist_dir(PERSIST_DIR)
    _, engine = await build_poisoned_store(PERSIST_DIR)
    print("[OK] Rebuilt and loaded in-memory engine.")
