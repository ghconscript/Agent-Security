...
async def run_memory_graft_experiment():
    ...
    # Enable experience pool. BM25 无需 embedding，可直接运行；若已配置 Chroma+embedding 可改为 CHROMA
    ...
    print("[OK] Experience pool enabled (BM25 retrieval; no embedding required)")
    ...
    if initial_count > 0:
        print(f"[WARN]  Found {initial_count} existing experience(s) in pool")
        print("[*] Clearing experience pool for clean experiment...")
        exp_manager.delete_all_exps()
        print("[OK] Experience pool cleared")
    else:
        print("[OK] Experience pool is empty - ready for experiment")
    ...
    try:
        ...
        print(f"[OK] Loaded {len(benign_seeds)} benign experience seeds")
        print(f"[OK] Loaded {len(poisoned_seeds)} poisoned experience seeds")
        print()
        ...
        print(f"[FAIL] Failed to load experience seeds: {e}")
        return
    ...
    print(f"[OK] Injected {len(benign_experiences)} benign experiences")
    print()
    ...
    print(f"[OK] Injected {len(poisoned_experiences)} poisoned experiences")
    print()
        ...
        try:
            ...
            if exps:
                ...
                if poisoned_found:
                    ...
                    print(f"  [OK] No poisoned experiences in results")
...
def main():
    ...
    try:
        ...
        print(f"\n\n[FAIL] Experiment failed with error: {e}")
        import traceback
        traceback.print_exc()
